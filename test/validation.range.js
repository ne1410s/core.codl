...
describe('@Validation.range', () => {
  ...
  it('max val -> formatted error message', () => {
    ...
    expect(numErrs[0]).to.equal('myNumberMax2 must be between -99 and 2');
  });
  ...
  it('bad number -> invalid (skip range check)', () => {
    ...
    expect(summary.errors['myNumberMax2'][0]).to.equal('myNumberMax2 is not a valid number');
  });
  ...
  it('bad date -> invalid (skip range check)', () => {
    ...
    expect(dateErrs).to.not.be.undefined;
    expect(dateErrs.length).to.equal(1);
    expect(dateErrs[0]).to.equal('myDate is not a valid date');
  });
});
